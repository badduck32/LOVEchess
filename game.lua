...
function gameClicked (fx, fy)
	...
	if highlightAt(x, y) and selectedPiece ~= nil and pieceAt(x, y) ~= selectedPiece then 
		...
		if pieceAt(x, y, whitesTurn) ~= nil then
			...
			createHighlights(pieceAt(x, y), false)
		else
			selectedPiece = nil 
		end
	end
...
function createHighlights(piece, checkingAttack, ax, ay)
	...
	if piece.type == 6 then 
		...
		if pieceAt(piece.x, piece.y + yDelta) ~= nil then 
			...
			if piece.firstMove and pieceAt(piece.x, piece.y + yDelta * 2) == nil and not checkingAttack then 
...
function createHighlightAt(index, x, y, checkingAttack, ax, ay)
...
function beingAttackedAt(x, y, white)
	...
	for i, piece in ipairs(arr) do
		if piece.dead == false then
			...
			createHighlights(piece, true, x, y)
		end
